...
public class UserServiceImpl implements UserService{
	...
	public boolean deleteUser(int id){
		...
		try{
			...
			//调用数据库工具类的beginTransaction方法，开启事务
			DBUtils.beginTransaction(conn);
			//调用dao中的deleteUser方法，进行数据库删除操作，结果赋值给删除结果变量
	...
	public Vector<UserVO> findUserByName(String name)
	{
		...
		try{
			...
			//将异常封装成自定义异常并抛出
			throw new ServiceException("根据用户名查询用户错误",e);
		}finally{
			DBUtils.closeConnection(conn);
		}
		//返回用户登录结果
	...
	public Vector<UserVO> findAll()
	{
		...
		//调用dao工厂类的getDao方法，取得指定类型的dao接口的实现类，并赋值给dao接口变量
		UserDao userMgrDao = (UserDao)DaoFactory.getDao(conn, EnumType.USER_DAO);
		//调用dao中的findAll方法，进行登录操作，结果赋值给登录结果变量
	...
	public boolean updateUser(UserVO user,int userid)
	{
		...
		try{
			...
			//调用数据库工具类的beginTransaction方法，开启事务
			DBUtils.beginTransaction(conn);
			//调用dao中的updateUser方法，进行数据库修改操作，结果赋值给修改结果变量
